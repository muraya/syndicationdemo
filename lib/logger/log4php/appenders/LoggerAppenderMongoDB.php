...
class LoggerAppenderMongoDB extends LoggerAppender
{
    ...
    public function activateOptions() {
        try {
            ...
            $this->closed = true;
            $this->warn(sprintf('Failed to connect to mongo deamon: %s',
                    $ex->getMessage()));
        } catch (InvalidArgumentException $ex) {
            $this->closed = true;
            $this->warn(sprintf('Error while selecting mongo database: %s',
                    $ex->getMessage()));
        } catch (Exception $ex) {
            $this->closed = true;
    ...
    public function append(LoggerLoggingEvent $event) {
        try {
            ...
            $this->warn(sprintf('Error while writing to mongo collection: %s',
                    $ex->getMessage()));
        }
    }
