...
class LoggerPatternParser
{
    ...
    private function addConverter($word, $modifiers, $option) {
        ...
        if (isset($this->converterMap[$word])) {
            ...
            trigger_error("log4php: Invalid keyword '%$word' in converison pattern. Ignoring keyword.",
                E_USER_WARNING);
        }
    }
    ...
    private function parseModifiers($modifiers) {
        ...
        if (!preg_match($pattern, $modifiers)) {
            trigger_error("log4php: Invalid modifier in conversion pattern: [$modifiers]. Ignoring modifier.",
                E_USER_WARNING);
            return $info;
        }
